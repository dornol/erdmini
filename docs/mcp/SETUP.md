...
erdmini server has a built-in MCP (Model Context Protocol) Streamable HTTP endpoint at `/mcp` with 89 tools. Server mode only — local mode does not support MCP.
