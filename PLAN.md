...
| # | 항목 | 설명 | 난이도 |
|---|------|------|--------|
| 4 | **Fit to Window** | 모든 테이블이 보이도록 자동 줌/이동 (현재 리셋만 있음) | 낮음 |
| 5 | **Command Palette (Cmd+K)** | 테이블/컬럼 빠른 검색 및 이동 | 중간 |
| 6 | **관계선 카디널리티 라벨** | crow's foot 위에 "1:N" 텍스트 표시 | 낮음 |
| 7 | **테이블 추가/삭제 애니메이션** | 카드 등장/사라짐이 갑작스러움 | 낮음 |
| 8 | **키보드 줌/팬** | Ctrl+Arrow로 팬, +/-로 줌 | 낮음 |
...
| 10 | **Mermaid/PlantUML 내보내기** | 텍스트 기반 다이어그램 포맷 공유용 | 낮음 |
...
| # | 항목 | 설명 | 난이도 |
|---|------|------|--------|
| 12 | **RelationLines 메모이제이션** | 매 렌더마다 모든 FK 경로 재계산 → 캐시 | 낮음 |
| 13 | **미니맵 렌더 스로틀** | 팬/줌 중 매 프레임 렌더 → throttle | 낮음 |
| 14 | **localStorage 저장 디바운스** | 테이블 드래그 중 매 이동마다 저장 → 디바운스 | 낮음 |
...
| 16 | **도메인 사용처 리포트** | 어떤 컬럼이 해당 도메인을 쓰는지 표시 | 낮음 |
| 17 | **CHECK 제약조건** | 컬럼 레벨 제약조건 UI | 중간 |
| 18 | **URL 기반 스키마 공유** | JSON → Base64 URL로 공유 | 중간 |
