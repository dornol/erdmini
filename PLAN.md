...
#### 미구현
- [ ] OIDC 인증 연동 (SvelteKit 모놀리식 — hooks.server.ts + API routes)
- [ ] 프로젝트 목록 페이지 (`/projects`)
- [ ] 인증 없는 경우 LocalStorage 모드 폴백

### Phase 5 — 실시간 협업

- [ ] WebSocket 기반 실시간 동기화 (SvelteKit 모놀리식)
- [ ] 다른 사용자 커서 / 선택 표시
...
- `hooks.server.ts`에서 세션 검증 (쿠키 기반)
- OIDC Provider (Keycloak, Auth0, Google 등) 연동
- 인증 없이도 LocalStorage 모드로 동작 (게스트 모드 유지)
- 서버 모드에서 OIDC 토큰 → 세션 쿠키 → API 보호

```
[브라우저] → [SvelteKit hooks.server.ts] → [OIDC Provider] → 토큰 발급
                     ↓ 세션 쿠키
