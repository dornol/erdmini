...
- **`mcp/`**: Stateless MCP server at `/mcp` route. Fresh `McpServer` per POST request. `Authorization: Bearer` token auth with scoped API key permissions. 69 tools: CRUD for tables/columns/FKs/memos/domains, FK update, unique key/index CRUD, column reorder, table duplicate, memo attach/detach, auto-layout, group/schema rename, schema read/export/lint/diagram/DDL, domain analysis/coverage/dictionary, schema namespace CRUD, snapshots, bulk ops (delete_tables, bulk_add_tables, bulk_add_columns, bulk_update_table_schema, bulk_update_tables, bulk_update_columns, bulk_assign_domain), search (get_project_by_name, get_table_by_name, search_columns, find_orphan_tables), table templates (list_table_templates, create_table_from_template), snapshot diff/migration SQL. Write ops call `notifyCollabSchemaChange()`.
...
- All utility functions in `src/lib/utils/` are pure; most have corresponding `.test.ts` files (54 test files, 2103 tests)
