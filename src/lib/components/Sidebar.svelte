<script lang="ts">
  ...
  import SidebarToolbar from './sidebar/SidebarToolbar.svelte';
  ...
  function getTableMeta(table: typeof erdStore.schema.tables[0]) {
    return getTableMetaFn(table, erdStore.schema.tables);
  }
    ...
    <VirtualList bind:this={virtualListRef} items={virtualRows} class="table-list thin-scrollbar" ondragover={onDragOverContainer}>
      {#snippet children(row: VirtualRow, _index: number)}
        ...
        {:else if row.type === 'new-group'}
          <SidebarGroupActions
            showInput={newGroupInput}
            groupName={newGroupName}
            oncreategroup={confirmNewGroup}
            oncancelgroup={cancelNewGroup}
            ongroupnameinput={(v) => { newGroupName = v; }}
