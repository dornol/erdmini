<script lang="ts">
  ...
  let canSubmit = $derived(
    selectedColumnIds.filter((id) => id !== '').length >= 1,
  );

  function submit() {
    if (!canSubmit) return;
    const ids = selectedColumnIds.filter((id) => id !== '');
    erdStore.addIndex(tableId, ids, isUnique, indexName.trim() || undefined);
    onclose();
  }
...
>
  <div class="modal">
    <div class="modal-header">
      <span class="modal-title">{m.idx_modal_title({ name: selectedTable?.name ?? '' })}</span>
    ...
    <div class="modal-body">
      <!-- Index name -->
      <div class="form-row">
        <label for="idx-name">{m.idx_name_placeholder()}</label>
        <input
          id="idx-name"
          class="text-input"
          bind:value={indexName}
          placeholder={m.idx_name_placeholder()}
        />
      </div>
      ...
      <div class="cols-section">
        <div class="cols-header">
          <span class="cols-label">{m.idx_column_select()}</span>
          <button class="btn-add-col" onclick={addColumn}>{m.idx_add_column()}</button>
        </div>
