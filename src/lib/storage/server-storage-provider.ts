...
export class ServerStorageProvider implements StorageProvider {
  async loadIndex(): Promise<ProjectIndex | null> {
    const res = await fetch(appPath('/api/storage/index'));
  ...
  async loadSchema(projectId: string): Promise<ERDSchema | null> {
    const res = await fetch(appPath(`/api/storage/schemas/${encodeURIComponent(projectId)}`));
    if (!res.ok) return null;
  ...
  async loadCanvasState(projectId: string): Promise<CanvasData | null> {
    const res = await fetch(appPath(`/api/storage/canvas/${encodeURIComponent(projectId)}`));
    if (!res.ok) return null;
  ...
  async listSnapshots(projectId: string): Promise<SchemaSnapshot[]> {
    const res = await fetch(appPath(`/api/storage/schemas/${encodeURIComponent(projectId)}/snapshots`));
    if (!res.ok) return [];
  ...
  async loadSnapshot(projectId: string, snapshotId: string): Promise<SchemaSnapshot | null> {
    const res = await fetch(appPath(`/api/storage/schemas/${encodeURIComponent(projectId)}/snapshots/${encodeURIComponent(snapshotId)}`));
    if (!res.ok) return null;
    return (await res.json()) as SchemaSnapshot;
  }
